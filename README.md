...
- api를 구축하여, 소통하는 것을 처음부터 직접 해본 것이 재밌었다.
- 1:N 필드와 M:N 필드를 직렬화하고 API에서 표현하는 과정에서 많은 오류를 겪고, 해결 방법을 알게되어서 좋았다.
- 인스턴스인지 쿼리셋인지에 따라 many=True옵션을 잘 넣어줘야한다고 생각되었다.
- 리뷰 POST 를 이용할 때 api를 쓰다보니 title로 데이터를 가져오거나 이런게 힘들었다. 내가 리뷰를 쓰는게 어디인지 헷갈리는 ??
- 해당 api를 프론트엔드로 넘겨서 어떻게 작업할지 좀 궁금하다.
